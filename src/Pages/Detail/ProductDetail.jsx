...
const ProductDetail = () => {
    ...
    const addToWhishList = async (product) => {
        if (setProceed) {
            try {
                const { data } = await axios.post(`${process.env.REACT_APP_ADD_WISHLIST}`, 
                    { _id: product._id }, 
                    {
                        headers: {
                            'Authorization': authToken
                ...
                // Update wishlist data properly
                const updatedWishlist = await axios.get(`${process.env.REACT_APP_GET_WISHLIST}`, {
                    headers: {
                        'Authorization': authToken
                    }
                })
                setWishlistData(updatedWishlist.data)
                toast.success("Added To Wishlist", { autoClose: 500, theme: 'colored' })
            }
            catch (error) {
                if (error.response?.status === 400) {
                    toast.error("Product already in wishlist", { autoClose: 500, theme: 'colored' })
                } else {
                    toast.error("Failed to add to wishlist", { autoClose: 500, theme: 'colored' })
                }
    ...
    const shareProduct = (product) => {
        ...
        const data = {
            text: product.name,
            title: "ShoStop",
            url: `https://ShoStop.vercel.app/Detail/type/${cat}/${id}`
        }
