                ...
                const options = {
                    ...
                    handler: async function (response) {
                        try {
                            ...
                            const result = await axios.post(`${process.env.REACT_APP_GET_PAYMENTVERIFICATION}`, verifyData);
                            
                            if (result.data.success) {
                                toast.success("Payment Successful!", { autoClose: 1500, theme: "colored" });
                                navigate(`/paymentsuccess?reference=${result.data.payment_id}`);
                            } else {
                                toast.error(result.data.msg || "Payment verification failed", { autoClose: 1500, theme: "colored" });
                            }
                        } catch (error) {
                            console.log(error);
                            toast.error(error.response?.data?.msg || "Payment verification failed", { autoClose: 1500, theme: "colored" });
                        }
                    },
                    prefill: {
                        name: userData.firstName + ' ' + userData.lastName,
